...
def download(WorkshopID, IsCollection=False):
    ...
    for _ in trange(LoadAttempts, desc="Loading PublishedFileDetails"):
        try:
            GetPublishedFileDetailsData = {
                "key": Key,
                "itemcount": 1,
                "publishedfileids[0]": WorkshopID,
            }
            GetPublishedFileDetailsRaw = requests.post(
                url=GetPublishedFileDetails, data=GetPublishedFileDetailsData
            )
            PublishedFileDetails = json.loads(GetPublishedFileDetailsRaw.text)[
                "response"
    ...
    if IsCollection:
        for _ in trange(LoadAttempts, desc="Loading CollectionDetails"):
            try:
                ...
                CollectionDetails = json.loads(GetCollectionDetailsRaw.text)[
                    "response"
                ]["collectiondetails"][0]
            except json.JSONDecodeError:
                time.sleep(30)
    ...
    if "file_url" in PublishedFileDetails and PublishedFileDetails["file_url"] != "":
        for _ in trange(LoadAttempts, desc="Loading PublishedFileDetailsFile"):
            try:
                GetUGCFileDetailsParametersFile = (
                    "?key="
                    + Key
                    + "&ugcid="
                ...
                GetUGCFileDetailsRawFile = requests.get(
                    url=GetUGCFileDetails + GetUGCFileDetailsParametersFile
                )
                if "data" in json.loads(GetUGCFileDetailsRawFile.text):
                    UGCFileDetailsFile = (json.loads(GetUGCFileDetailsRawFile.text))[
                        "data"
                    ]
            except json.JSONDecodeError:
                time.sleep(30)
            ...
            for _ in trange(
                LoadAttempts, desc="Loading PublishedFileDetailsFile from Fallback ID"
            ):
                try:
                    GetUGCFileDetailsParametersFile = (
                        "?key="
                    ...
                    if "data" in json.loads(GetUGCFileDetailsRawFile.text):
                        UGCFileDetailsFile = (
                            json.loads(GetUGCFileDetailsRawFile.text)
                        )["data"]
                except json.JSONDecodeError:
                    time.sleep(30)
                    continue
    ...
    if (
        "preview_url" in PublishedFileDetails
        and PublishedFileDetails["preview_url"] != ""
    ):
        for _ in trange(LoadAttempts, desc="Loading PublishedFileDetailsPreview"):
            try:
                GetUGCFileDetailsParametersPreview = (
                    "?key="
                    + Key
                    + "&ugcid="
                ...
                GetUGCFileDetailsRawPreview = requests.get(
                    url=GetUGCFileDetails + GetUGCFileDetailsParametersPreview
                )
                if "data" in json.loads(GetUGCFileDetailsRawPreview.text):
                    UGCFileDetailsPreview = (
                        json.loads(GetUGCFileDetailsRawPreview.text)
                    )["data"]
            except json.JSONDecodeError:
                time.sleep(30)
                continue
            ...
            for _ in trange(
                LoadAttempts,
                desc="Loading PublishedFileDetailsPreview from Fallback ID",
            ):
                try:
                    GetUGCFileDetailsParametersPreview = (
                        "?key="
    ...
    ) + datetime.timedelta(seconds=PublishedFileDetails["time_created"])
    ...
    if (
        ...
        with requests.get(UGCFileDetailsPreview["url"]) as RawPreviewContent, open(
            LocalPreviewLocation, "wb"
        ) as LocalPreview, open(
            sanitize_filepath(LocalPreviewLocation + ".headers.json"), "w"
        ) as PreviewContentHeadersOutput:
            LocalPreview.write(RawPreviewContent.content)
            json.dump(
                dict(RawPreviewContent.headers),
                PreviewContentHeadersOutput,
                sort_keys=True,
                indent=4,
            )

    # Save JSON Responses
    PublishedFileDetailsOutput = open(
        sanitize_filepath(OutputDirectory + "/PublishedFileDetails.json"), "w"
    )
    json.dump(
        PublishedFileDetails, PublishedFileDetailsOutput, sort_keys=True, indent=4
    )
    if "file_url" in PublishedFileDetails and PublishedFileDetails["file_url"] != "":
        UGCFileDetailsOutputFile = open(
            sanitize_filepath(OutputDirectory + "/UGCFileDetails.file.json"), "w"
        )
        json.dump(
            UGCFileDetailsFile, UGCFileDetailsOutputFile, sort_keys=True, indent=4
        )
    if (
        "preview_url" in PublishedFileDetails
        and PublishedFileDetails["preview_url"] != ""
    ):
        UGCFileDetailsOutputPreview = open(
            sanitize_filepath(OutputDirectory + "/UGCFileDetails.preview.json"), "w"
        )
        json.dump(
            UGCFileDetailsPreview, UGCFileDetailsOutputPreview, sort_keys=True, indent=4
        )
    if IsCollection:
        CollectionDetailsOutput = open(
            sanitize_filepath(OutputDirectory + "/CollectionDetails.json"), "w"
        )
...
def ExternalDetailsDownload(PublishedFileDetails, IsCollection=False):
    CreatorAppID = str(PublishedFileDetails["creator_appid"])
    ConsumerAppID = str(PublishedFileDetails["consumer_appid"])
    LoadAttempts = 1000
    WorkshopID = PublishedFileDetails["publishedfileid"]
    GetCollectionDetails = (
        "https://api.steampowered.com/ISteamRemoteStorage/GetCollectionDetails/v1/"
    )
    GetUGCFileDetails = (
    ...
    # File Directory Variables
    if (
        "file_url" in PublishedFileDetails
        and PublishedFileDetails["file_url"] != ""
        and "UGCFileDetailsFile" in vars()
    ):
        FileName = UGCFileDetailsFile["filename"]
        FileDirectories = FileName.split("/")
        FileName = FileDirectories[-1]
    ...
    if (
        "preview_url" in PublishedFileDetails
        and PublishedFileDetails["preview_url"] != ""
        and "UGCFileDetailsPreview" in vars()
    ):
        PreviewName = UGCFileDetailsPreview["filename"]
        PreviewDirectories = PreviewName.split("/")
        PreviewName = PreviewDirectories[-1]
    ...
    # Output Directory Creation
    OutputDirectory = (
        "./Workshop-Downloads-No-SteamID-Folders/"
        + ConsumerAppID
        + "/"
        + DateCreatedISO8601
        + " - "
